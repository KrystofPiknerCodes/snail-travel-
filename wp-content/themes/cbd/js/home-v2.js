...
(function () {
  ...
  // Snail Travel nav: same 6 links as the Astro site's header (web/src/layouts/Base.astro)
  // -- plain rows, no submenus, no Fitzroy country/inspiration dropdowns and no US phone pill.
  var SNAIL_NAV = [
    { label: 'O nás', href: '/o-nas/' },
    { label: 'Destinace', href: '/destinace/' },
    { label: 'Reference', href: '/reference/' },
    { label: 'Zážitky', href: '/#zazitky' },
    { label: 'Kontakt', href: '/#kontakt' },
    { label: 'Golf', href: 'https://www.snailtravelgolf.cz/', external: true }
  ];
  function buildMenu(menu) {
    var rows = SNAIL_NAV.map(function (it) {
      return '<a class="nsrow" href="' + it.href + '"' + (it.external ? ' target="_blank" rel="noopener"' : '') +
             '><span class="nsitem">' + it.label + '</span>' + ARROWS + '</a>';
    }).join('');
    menu.innerHTML =
      ...
      '</button>' +
      '<nav class="v2menu__list">' + rows + '</nav>' +
      '<a href="/#kontakt" class="v2btn v2btn--gold v2menu__cta"><span>Kontaktujte nás</span></a>' +
      '</aside>';
